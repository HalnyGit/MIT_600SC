...
def graphRemovedErr(points, kvals = [25, 50, 75, 100, 125, 150], cutoff = 0.1):
    ...
    sets = (trainingSet, holdoutSet)
    trainingSetError = {}
    holdoutSetError = {}
    totDist = 0.0
    for i in range(len(sets)):
        for kval in kvals:
            clusters, maxDist = kmeans(sets[i], kval, cutoff, County)
            totalError = 0.0
            for cluster in clusters:
                for p in cluster.getPoints():
                    distance = p.distance(cluster.getCentroid())
                    totalError += distance**2	
            if i == 0:
		        trainingSetError[kval] = totalError
            elif i == 1:
                holdoutSetError[kval] = totalError

    ratioError = {}
    for kval in kvals:
        ratioError[kval] = holdoutSetError[kval]/trainingSetError[kval]

    pylab.figure()
    pylab.title('Total error of trainingset and holdoutset according to num. of clusters')
    pylab.xlabel('Number of clusters')
    pylab.ylabel('Error')
    pylab.plot(kvals, trainingSetError.values() , label = 'Training set')
    pylab.plot(kvals, holdoutSetError.values(), label = 'Holdout set')
    pylab.legend(loc = 'best')
	
    pylab.figure()
    pylab.title('Ratio  Holdout set error over training set error')
    pylab.xlabel('Number of clusters')
    pylab.ylabel('Ratio')
    pylab.plot(kvals, ratioError.values(), label = 'Ratio')
    pylab.legend(loc = 'best')
    pylab.show()	
	
print graphRemovedErr(points)
